...
export function RecentSessionsList({
  ...
  if (items.length === 0) {
    return (
      <div
        className="rounded-lg border border-dashed border-slate-700/90 bg-slate-900/30 px-4 py-6 text-center"
        role="status"
      >
        <p className="text-sm font-medium text-slate-400">No recent sessions yet</p>
        <p className="mt-1 text-xs leading-relaxed text-slate-500">
          Join or create a session from the dashboard; IDs you use will appear here for quick rejoin.
        </p>
      </div>
    );
  }
    ...
    <ul className="space-y-2" aria-label="Recent sessions">
      {items.map((item) => {
        ...
        return (
          <li
            key={item.sessionId}
            className="flex min-w-0 items-stretch gap-2 rounded-lg border border-slate-800/90 bg-slate-900/40"
          >
            <button
              type="button"
              onClick={() => onRowClickFill(item.sessionId)}
              disabled={disabled}
              className="min-w-0 flex-1 px-3 py-2.5 text-left transition hover:bg-slate-800/50 disabled:pointer-events-none disabled:opacity-50"
            >
              <span className="block truncate text-sm font-medium text-slate-100">{item.sessionName}</span>
              <span className="mt-0.5 block truncate font-mono text-xs text-slate-500">{item.sessionId}</span>
            </button>
