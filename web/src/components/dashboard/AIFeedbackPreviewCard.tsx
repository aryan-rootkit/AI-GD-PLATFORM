...
import type { AIFeedbackPreviewState } from '@/hooks/useAIFeedbackPreview';
import { Sparkles } from 'lucide-react';

type Props = {
  state: AIFeedbackPreviewState;
};

export function AIFeedbackPreviewCard({ state }: Props) {
  return (
    <section
      ...
      {state.status === 'ready' && (
        <div className="mt-5 space-y-5">
          <div>
            ...
            {state.preview.lastSessionScore != null ? (
              <p className="mt-1 text-3xl font-bold tabular-nums text-violet-300">
                {state.preview.lastSessionScore}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1.5 text-sm leading-relaxed text-slate-300">{state.preview.strength}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Improvement</p>
              <p className="mt-1.5 text-sm leading-relaxed text-slate-300">{state.preview.improvement}</p>
            </div>
          </div>
          {state.source === 'live' && (
            <p className="text-xs text-slate-500">
              From your most recent ended session.{' '}
              <Link href="/dashboard/history" className="text-violet-400 hover:underline">
                View history
              </Link>
            </p>
          )}
