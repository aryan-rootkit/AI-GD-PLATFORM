...
const AuthForm = ({ type }: { type: FormType }) => {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      const localDev = isLocalDevClient();
    ...
    <div className="card-border lg:min-w-[566px]">
      <div className="flex flex-col gap-6 card py-14 px-10">
        <div className="flex flex-col items-center gap-3">
          <AthenaLogo size="lg" priority />
          <p className="text-sm text-light-400 text-center max-w-md">
            {ATHENA_DESCRIPTION}
          </p>
          <p className="text-xs text-light-600 text-center max-w-sm">{ATHENA_FOCUS}</p>
        </div>

        <h3 className="text-center">
          {isSignIn ? "Sign in to your training hub" : "Create your ATHENA account"}
        </h3>
